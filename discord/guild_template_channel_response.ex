defmodule Discord.GuildTemplateChannelResponse do
  @moduledoc """
  ...
  def __fields__(:t) do
    [
      ...
      id: {:union, [{:integer, "int32"}, :null]},
      name: {:union, [:string, :null]},
      nsfw: :boolean,
      parent_id: {:union, [{:string, "snowflake"}, :null]},
      permission_overwrites: [union: [{Discord.ChannelPermissionOverwriteResponse, :t}, :null]],
      position: {:union, [{:integer, "int32"}, :null]},
      rate_limit_per_user: {:integer, "int32"},
      template: :string,
      theme_color: {:union, [{:integer, "int32"}, :null]},
      topic: {:union, [:string, :null]},
      type: {:enum, [0, 2, 4]},
      user_limit: {:integer, "int32"}
    ]
  end
end
